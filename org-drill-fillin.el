...
(defgroup org-drill-fillin nil
  "Options concerning interactive drill sessions in Org mode (org-drill-fillin)."
  :tag "Org-Drill-Fillin"
  :group 'org-link)



(defcustom org-drill-fillin-question-tag
  "drill"
  "Tag which topics must possess in order to be identified as review topics
by `org-drill-fillin'."
  :group 'org-drill-fillin
  :type 'string)


(defcustom org-drill-fillin-maximum-items-per-session
  30
...
(defcustom org-drill-fillin-maximum-duration
  20
...
(defcustom org-drill-fillin-item-count-includes-failed-items-p
  nil
...
(defcustom org-drill-fillin-failure-quality
  2
...
(defcustom org-drill-fillin-forgetting-index
  10
...
(defcustom org-drill-fillin-leech-failure-threshold
  15
...
(defcustom org-drill-fillin-leech-method
  'skip
...
(defface org-drill-fillin-visible-cloze-face
  '((t (:foreground "darkseagreen")))
  "The face used to hide the contents of cloze phrases."
  :group 'org-drill-fillin)


(defface org-drill-fillin-visible-cloze-hint-face
  '((t (:foreground "dark slate blue")))
  "The face used to hide the contents of cloze phrases."
  :group 'org-drill-fillin)


(defface org-drill-fillin-hidden-cloze-face
  '((t (:foreground "deep sky blue" :background "blue")))
  "The face used to hide the contents of cloze phrases."
  :group 'org-drill-fillin)


(defcustom org-drill-fillin-use-visible-cloze-face-p
  nil
...
(defcustom org-drill-fillin-hide-item-headings-p
  nil
...
(defcustom org-drill-fillin-new-count-color
  "royal blue"
...
(defcustom org-drill-fillin-mature-count-color
  "green"
...
(defcustom org-drill-fillin-failed-count-color
  "red"
...
(defcustom org-drill-fillin-done-count-color
  "sienna"
...
(defcustom org-drill-fillin-left-cloze-delimiter
  "["
  "String used within org buffers to delimit cloze deletions."
  :group 'org-drill-fillin
  :type 'string)

(defcustom org-drill-fillin-right-cloze-delimiter
  "]"
  "String used within org buffers to delimit cloze deletions."
  :group 'org-drill-fillin
  :type 'string)
...
(setplist 'org-drill-fillin-cloze-overlay-defaults
          `(display ,(format "%s...%s"
                             org-drill-fillin-left-cloze-delimiter
                             org-drill-fillin-right-cloze-delimiter)
                    face org-drill-fillin-hidden-cloze-face
                    window t))

(setplist 'org-drill-fillin-hidden-text-overlay
          '(invisible t))

(setplist 'org-drill-fillin-replaced-text-overlay
          '(display "Replaced text"
                    face default
                    window t))

(add-hook 'org-font-lock-set-keywords-hook 'org-drill-fillin-add-cloze-fontification)


(defvar org-drill-fillin-hint-separator "||"
...
(defun org-drill-fillin--compute-cloze-regexp ()
  (concat "\\("
          (regexp-quote org-drill-fillin-left-cloze-delimiter)
          "[[:cntrl:][:graph:][:space:]]+?\\)\\(\\|"
          (regexp-quote org-drill-fillin-hint-separator)
          ".+?\\)\\("
          (regexp-quote org-drill-fillin-right-cloze-delimiter)
          "\\)"))

(defun org-drill-fillin--compute-cloze-keywords ()
  (list (list (org-drill-fillin--compute-cloze-regexp)
              (copy-list '(1 'org-drill-fillin-visible-cloze-face nil))
              (copy-list '(2 'org-drill-fillin-visible-cloze-hint-face t))
              (copy-list '(3 'org-drill-fillin-visible-cloze-face nil))
              )))

(defvar-local org-drill-fillin-cloze-regexp
  (org-drill-fillin--compute-cloze-regexp))


(defvar-local org-drill-fillin-cloze-keywords
  (org-drill-fillin--compute-cloze-keywords))


;; Variables defining what keys can be pressed during drill sessions to quit the
;; session, edit the item, etc.
(defvar org-drill-fillin--quit-key ?q
  "If this character is pressed during a drill session, quit the session.")
(defvar org-drill-fillin--edit-key ?e
  "If this character is pressed during a drill session, suspend the session
with the cursor at the current item..")
(defvar org-drill-fillin--help-key ??
  "If this character is pressed during a drill session, show help.")
(defvar org-drill-fillin--skip-key ?s
  "If this character is pressed during a drill session, skip to the next
item.")
(defvar org-drill-fillin--tags-key ?t
  "If this character is pressed during a drill session, edit the tags for
the current item.")


(defcustom org-drill-fillin-card-type-alist
  '((nil org-drill-fillin-present-simple-card)
    ("simple" org-drill-fillin-present-simple-card)
    ("simpletyped" org-drill-fillin-present-simple-card-with-typed-answer)
    ("twosided" org-drill-fillin-present-two-sided-card nil t)
    ("multisided" org-drill-fillin-present-multi-sided-card nil t)
    ("hide1cloze" org-drill-fillin-present-multicloze-hide1)
    ("hide2cloze" org-drill-fillin-present-multicloze-hide2)
    ("show1cloze" org-drill-fillin-present-multicloze-show1)
    ("show2cloze" org-drill-fillin-present-multicloze-show2)
    ("multicloze" org-drill-fillin-present-multicloze-hide1)
    ("hidefirst" org-drill-fillin-present-multicloze-hide-first)
    ("hidelast" org-drill-fillin-present-multicloze-hide-last)
    ("hide1_firstmore" org-drill-fillin-present-multicloze-hide1-firstmore)
    ("show1_lastmore" org-drill-fillin-present-multicloze-show1-lastmore)
    ("show1_firstless" org-drill-fillin-present-multicloze-show1-firstless)
    ("conjugate"
     org-drill-fillin-present-verb-conjugation
     org-drill-fillin-show-answer-verb-conjugation)
    ("decline_noun"
     org-drill-fillin-present-noun-declension
     org-drill-fillin-show-answer-noun-declension)
    ("spanish_verb" org-drill-fillin-present-spanish-verb)
    ("translate_number" org-drill-fillin-present-translate-number))
...
(defcustom org-drill-fillin-scope
  'file
...
(defcustom org-drill-fillin-match
  nil
...
(defcustom org-drill-fillin-save-buffers-after-drill-sessions-p
  t
...
(defcustom org-drill-fillin-spaced-repetition-algorithm
  'sm5
...
(defcustom org-drill-fillin-optimal-factor-matrix
  nil
  "Obsolete and will be removed in future. The SM5 optimal factor
matrix data is now stored in the variable
`org-drill-fillin-sm5-optimal-factor-matrix'."
  :group 'org-drill-fillin
  :type 'sexp)


(defvar org-drill-fillin-sm5-optimal-factor-matrix
  nil
...
(defun org-drill-fillin--transfer-optimal-factor-matrix ()
  (if (and org-drill-fillin-optimal-factor-matrix
           (null org-drill-fillin-sm5-optimal-factor-matrix))
      (setq org-drill-fillin-sm5-optimal-factor-matrix
            org-drill-fillin-optimal-factor-matrix)))

(add-hook 'after-init-hook 'org-drill-fillin--transfer-optimal-factor-matrix)


(defcustom org-drill-fillin-sm5-initial-interval
  4.0
...
(defcustom org-drill-fillin-add-random-noise-to-intervals-p
  nil
...
(defcustom org-drill-fillin-adjust-intervals-for-early-and-late-repetitions-p
  nil
...
(defcustom org-drill-fillin-cloze-text-weight
  4
...
(defcustom org-drill-fillin-cram-hours
  12
...
;;;   ORG-DRILL-FILLIN-DAYS-BEFORE-OLD days after their last
...
;;;   ORG-DRILL-FILLIN-DAYS-BEFORE-OLD.
;;; - OVERDUE items are past their scheduled review date by more than
;;;   LAST-INTERVAL * (ORG-DRILL-FILLIN-OVERDUE-INTERVAL-FACTOR - 1) days,
;;;   regardless of young/old status.


(defcustom org-drill-fillin-days-before-old
  10
...
(defcustom org-drill-fillin-overdue-interval-factor
  1.2
  "An item is considered overdue if its scheduled review date is
more than (ORG-DRILL-FILLIN-OVERDUE-INTERVAL-FACTOR - 1) * LAST-INTERVAL
...
(defcustom org-drill-fillin-learn-fraction
  0.5
...
(defcustom org-drill-fillin-cloze-length-matches-hidden-text-p
  nil
...
(defvar *org-drill-fillin-session-qualities* nil)
(defvar *org-drill-fillin-start-time* 0)
(defvar *org-drill-fillin-new-entries* nil)
(defvar *org-drill-fillin-dormant-entry-count* 0)
(defvar *org-drill-fillin-due-entry-count* 0)
(defvar *org-drill-fillin-overdue-entry-count* 0)
(defvar *org-drill-fillin-due-tomorrow-count* 0)
(defvar *org-drill-fillin-overdue-entries* nil
  "List of markers for items that are considered 'overdue', based on
the value of ORG-DRILL-FILLIN-OVERDUE-INTERVAL-FACTOR.")
(defvar *org-drill-fillin-young-mature-entries* nil
  "List of markers for mature entries whose last inter-repetition
interval was <= ORG-DRILL-FILLIN-DAYS-BEFORE-OLD days.")
(defvar *org-drill-fillin-old-mature-entries* nil
  "List of markers for mature entries whose last inter-repetition
interval was greater than ORG-DRILL-FILLIN-DAYS-BEFORE-OLD days.")
(defvar *org-drill-fillin-failed-entries* nil)
(defvar *org-drill-fillin-again-entries* nil)
(defvar *org-drill-fillin-done-entries* nil)
(defvar *org-drill-fillin-current-item* nil
  "Set to the marker for the item currently being tested.")
(defvar *org-drill-fillin-cram-mode* nil
  "Are we in 'cram mode', where all items are considered due
for review unless they were already reviewed in the recent past?")
(defvar org-drill-fillin-scheduling-properties
...
(defvar org-drill-fillin--lapse-very-overdue-entries-p nil
...
(put 'org-drill-fillin-question-tag 'safe-local-variable 'stringp)
(put 'org-drill-fillin-maximum-items-per-session 'safe-local-variable
     '(lambda (val) (or (integerp val) (null val))))
(put 'org-drill-fillin-maximum-duration 'safe-local-variable
     '(lambda (val) (or (integerp val) (null val))))
(put 'org-drill-fillin-failure-quality 'safe-local-variable 'integerp)
(put 'org-drill-fillin-forgetting-index 'safe-local-variable 'integerp)
(put 'org-drill-fillin-leech-failure-threshold 'safe-local-variable 'integerp)
(put 'org-drill-fillin-leech-method 'safe-local-variable
     '(lambda (val) (memq val '(nil skip warn))))
(put 'org-drill-fillin-use-visible-cloze-face-p 'safe-local-variable 'booleanp)
(put 'org-drill-fillin-hide-item-headings-p 'safe-local-variable 'booleanp)
(put 'org-drill-fillin-spaced-repetition-algorithm 'safe-local-variable
     '(lambda (val) (memq val '(simple8 sm5 sm2))))
(put 'org-drill-fillin-sm5-initial-interval 'safe-local-variable 'floatp)
(put 'org-drill-fillin-add-random-noise-to-intervals-p 'safe-local-variable 'booleanp)
(put 'org-drill-fillin-adjust-intervals-for-early-and-late-repetitions-p
     'safe-local-variable 'booleanp)
(put 'org-drill-fillin-cram-hours 'safe-local-variable 'integerp)
(put 'org-drill-fillin-learn-fraction 'safe-local-variable 'floatp)
(put 'org-drill-fillin-days-before-old 'safe-local-variable 'integerp)
(put 'org-drill-fillin-overdue-interval-factor 'safe-local-variable 'floatp)
(put 'org-drill-fillin-scope 'safe-local-variable
     '(lambda (val) (or (symbolp val) (listp val))))
(put 'org-drill-fillin-match 'safe-local-variable
     '(lambda (val) (or (stringp val) (null val))))
(put 'org-drill-fillin-save-buffers-after-drill-sessions-p 'safe-local-variable 'booleanp)
(put 'org-drill-fillin-cloze-text-weight 'safe-local-variable
     '(lambda (val) (or (null val) (integerp val))))
(put 'org-drill-fillin-left-cloze-delimiter 'safe-local-variable 'stringp)
(put 'org-drill-fillin-right-cloze-delimiter 'safe-local-variable 'stringp)
...
(defun org-map-drill-entries (func &optional scope drill-match &rest skip)
  ...
  (let ((org-drill-fillin-scope (or scope org-drill-fillin-scope))
        (org-drill-fillin-match (or drill-match org-drill-fillin-match)))
    (apply 'org-map-entries func
           (concat "+" org-drill-fillin-question-tag
                   (if (and (stringp org-drill-fillin-match)
                            (not (member '(?+ ?- ?|) (elt org-drill-fillin-match 0))))
                       "+" "")
                   (or org-drill-fillin-match ""))
           (case org-drill-fillin-scope
             (file nil)
             (file-no-restriction 'file)
             (directory
              (directory-files (file-name-directory (buffer-file-name))
                               t "\\.org$"))
             (t org-drill-fillin-scope))
           skip)))


(defmacro with-hidden-cloze-text (&rest body)
  `(progn
     (org-drill-fillin-hide-clozed-text)
     (unwind-protect
         (progn
           ,@body)
       (org-drill-fillin-unhide-clozed-text))))


(defmacro with-hidden-cloze-hints (&rest body)
  `(progn
     (org-drill-fillin-hide-cloze-hints)
     (unwind-protect
         (progn
           ,@body)
       (org-drill-fillin-unhide-text))))


(defmacro with-hidden-comments (&rest body)
  `(progn
     (if org-drill-fillin-hide-item-headings-p
         (org-drill-fillin-hide-heading-at-point))
     (org-drill-fillin-hide-comments)
     (unwind-protect
         (progn
           ,@body)
       (org-drill-fillin-unhide-text))))
...
(defun org-drill-fillin-days-since-last-review ()
...
(defun org-drill-fillin-hours-since-last-review ()
  "Like `org-drill-fillin-days-since-last-review', but return value is
...
(defun org-drill-fillin-entry-p (&optional marker)
...
situation use `org-part-of-drill-entry-p'."
  (save-excursion
    (when marker
      (org-drill-fillin-goto-entry marker))
    (member org-drill-fillin-question-tag (org-get-local-tags))))


(defun org-drill-fillin-goto-entry (marker)
...
or a subheading within a drill item?"
  (or (org-drill-fillin-entry-p)
      ;; Does this heading INHERIT the drill tag
      (member org-drill-fillin-question-tag (org-get-tags-at))))


(defun org-drill-fillin-goto-drill-entry-heading ()
...
drill entry."
  ...
  (while (not (org-drill-fillin-entry-p))
    (unless (org-up-heading-safe)
      (error "Cannot find a parent heading that is marked as a drill entry"))))



(defun org-drill-fillin-entry-leech-p ()
  "Is the current entry a 'leech item'?"
  (and (org-drill-fillin-entry-p)
       (member "leech" (org-get-local-tags))))


;; (defun org-drill-fillin-entry-due-p ()
;;   (cond
;;    (*org-drill-fillin-cram-mode*
;;     (let ((hours (org-drill-fillin-hours-since-last-review)))
;;       (and (org-drill-fillin-entry-p)
;;            (or (null hours)
;;                (>= hours org-drill-fillin-cram-hours)))))
;;    (t
;;     (let ((item-time (org-get-scheduled-time (point))))
;;       (and (org-drill-fillin-entry-p)
;;            (or (not (eql 'skip org-drill-fillin-leech-method))
;;                (not (org-drill-fillin-entry-leech-p)))
...
(defun org-drill-fillin-entry-days-overdue ()
  "Returns:
...
- A positive integer - item is scheduled that many days in the past."
  (cond
   ...
   (t
    (let ((item-time (org-get-scheduled-time (point))))
      (cond
       ((or (not (org-drill-fillin-entry-p))
            (and (eql 'skip org-drill-fillin-leech-method)
                 (org-drill-fillin-entry-leech-p)))
        nil)
...
(defun org-drill-fillin-entry-overdue-p (&optional days-overdue last-interval)
...
from the entry at point."
  (unless days-overdue
    (setq days-overdue (org-drill-fillin-entry-days-overdue)))
  (unless last-interval
    (setq last-interval (org-drill-fillin-entry-last-interval 1)))
  (and (numberp days-overdue)
       (> days-overdue 1)               ; enforce a sane minimum 'overdue' gap
       ;;(> due org-drill-fillin-days-before-overdue)
       (> (/ (+ days-overdue last-interval 1.0) last-interval)
          org-drill-fillin-overdue-interval-factor)))



(defun org-drill-fillin-entry-due-p ()
  (let ((due (org-drill-fillin-entry-days-overdue)))
    (and (not (null due))
         (not (minusp due)))))


(defun org-drill-fillin-entry-new-p ()
  (and (org-drill-fillin-entry-p)
       (let ((item-time (org-get-scheduled-time (point))))
         (null item-time))))


(defun org-drill-fillin-entry-last-quality (&optional default)
...
(defun org-drill-fillin-entry-failure-count ()
...
(defun org-drill-fillin-entry-average-quality (&optional default)
  (let ((val (org-entry-get (point) "DRILL_AVERAGE_QUALITY")))
    (if val
        (string-to-number val)
      (or default nil))))

(defun org-drill-fillin-entry-last-interval (&optional default)
  (let ((val (org-entry-get (point) "DRILL_LAST_INTERVAL")))
    (if val
        (string-to-number val)
      (or default 0))))

(defun org-drill-fillin-entry-repeats-since-fail (&optional default)
  (let ((val (org-entry-get (point) "DRILL_REPEATS_SINCE_FAIL")))
    (if val
        (string-to-number val)
      (or default 0))))

(defun org-drill-fillin-entry-total-repeats (&optional default)
  (let ((val (org-entry-get (point) "DRILL_TOTAL_REPEATS")))
    (if val
        (string-to-number val)
      (or default 0))))

(defun org-drill-fillin-entry-ease (&optional default)
...
(defun org-drill-fillin-random-dispersal-factor ()
...
(defun org-drill-fillin-early-interval-factor (optimal-factor
                                        optimal-interval
                                        days-ahead)
  "Arguments:
...
(defun org-drill-fillin-get-item-data ()
...
"
  (let ((learn-str (org-entry-get (point) "LEARN_DATA"))
        (repeats (org-drill-fillin-entry-total-repeats :missing)))
    (cond
     (learn-str
      (let ((learn-data (or (and learn-str
        ...
        (list (nth 0 learn-data)        ; last interval
              ...
              (org-drill-fillin-entry-failure-count)
              (nth 1 learn-data)
              (org-drill-fillin-entry-last-quality)
              (nth 2 learn-data)        ; EF
              )))
     ((not (eql :missing repeats))
      (list (org-drill-fillin-entry-last-interval)
            (org-drill-fillin-entry-repeats-since-fail)
            (org-drill-fillin-entry-failure-count)
            (org-drill-fillin-entry-total-repeats)
            (org-drill-fillin-entry-average-quality)
            (org-drill-fillin-entry-ease)))
     (t  ; virgin item
      (list 0 0 0 0 nil nil)))))


(defun org-drill-fillin-store-item-data (last-interval repeats failures
                                                total-repeats meanq
                                                ease)
...
- OF-MATRIX is not modified."
  ...
  (if (<= quality org-drill-fillin-failure-quality)
    ...
    (let* ((next-ef (modify-e-factor ef quality))
           (interval
            (cond
             ...
             ((= n 2)
              (cond
               (org-drill-fillin-add-random-noise-to-intervals-p
                (case quality
                  (5 6)
                  (4 4)
                  (3 3)
      ...
      (list (if org-drill-fillin-add-random-noise-to-intervals-p
                (+ last-interval (* (- interval last-interval)
                                    (org-drill-fillin-random-dispersal-factor)))
              interval)
            (1+ n)
            next-ef
...
(defun initial-optimal-factor-sm5 (n ef)
  (if (= 1 n)
      org-drill-fillin-sm5-initial-interval
    ef))
...
(defun determine-next-interval-sm5 (last-interval n ef quality
  ...
  (assert (and (>= quality 0) (<= quality 5)))
  (unless of-matrix
    (setq of-matrix org-drill-fillin-sm5-optimal-factor-matrix))
  ...
  (let ((next-ef (modify-e-factor ef quality))
        ...
        (new-of (modify-of (get-optimal-factor-sm5 n ef of-matrix)
                           quality org-drill-fillin-learn-fraction))
        (interval nil))
    (when (and org-drill-fillin-adjust-intervals-for-early-and-late-repetitions-p
               delta-days (minusp delta-days))
      (setq new-of (org-drill-fillin-early-interval-factor
    ...
    (cond
     ...
     ((<= quality org-drill-fillin-failure-quality)
     ...
     (t
      (setq interval (inter-repetition-interval-sm5
                      last-interval n ef of-matrix))
      (if org-drill-fillin-add-random-noise-to-intervals-p
          (setq interval (* interval (org-drill-fillin-random-dispersal-factor))))
      (list interval
            (1+ n)
            ef
            failures
...
(defun org-drill-fillin-simple8-first-interval (failures)
  "Arguments:
...
(defun org-drill-fillin-simple8-interval-factor (ease repetition)
  "Arguments:
...
multiplied to give the next interval. Corresponds to `RF' or `OF'."
  (+ 1.2 (* (- ease 1.2) (expt org-drill-fillin-learn-fraction (log repetition 2)))))


(defun org-drill-fillin-simple8-quality->ease (quality)
...
- AVERAGE-QUALITY
- TOTAL-REPEATS.
See the documentation for `org-drill-fillin-get-item-data' for a description of these."
  (assert (>= repeats 0))
  (assert (and (>= quality 0) (<= quality 5)))
  ...
  (let ((next-interval nil))
    ...
    (cond
     ...
     ((or (zerop repeats)
          (zerop last-interval))
      (setf next-interval (org-drill-fillin-simple8-first-interval failures))
      (incf repeats)
      (incf totaln))
     (t
      (let* ((use-n
              (if (and
                   org-drill-fillin-adjust-intervals-for-early-and-late-repetitions-p
                   (numberp delta-days) (plusp delta-days)
                   (plusp last-interval))
                  (+ repeats (min 1 (/ delta-days last-interval 1.0)))
                repeats))
             (factor (org-drill-fillin-simple8-interval-factor
                      (org-drill-fillin-simple8-quality->ease meanq) use-n))
             (next-int (* last-interval factor)))
        (when (and org-drill-fillin-adjust-intervals-for-early-and-late-repetitions-p
                   (numberp delta-days) (minusp delta-days))
          ;; The item was reviewed earlier than scheduled.
          (setf factor (org-drill-fillin-early-interval-factor
        ...
        (incf totaln))))
    (list
     (if (and org-drill-fillin-add-random-noise-to-intervals-p
              (plusp next-interval))
         (* next-interval (org-drill-fillin-random-dispersal-factor))
       next-interval)
     repeats
     (org-drill-fillin-simple8-quality->ease meanq)
     failures
     meanq
     totaln
     )))
...
(defun org-drill-fillin-smart-reschedule (quality &optional days-ahead)
...
item will be scheduled exactly this many days into the future."
  (let ((delta-days (- (time-to-days (current-time))
                       (time-to-days (or (org-get-scheduled-time (point))
                                         (current-time)))))
        (ofmatrix org-drill-fillin-sm5-optimal-factor-matrix)
    ...
    (destructuring-bind (last-interval repetitions failures
        ...
        (org-drill-fillin-get-item-data)
      (destructuring-bind (next-interval repetitions ease
                                         failures meanq total-repeats
                                         &optional new-ofmatrix)
          (case org-drill-fillin-spaced-repetition-algorithm
        ...
        (org-drill-fillin-store-item-data next-interval repetitions failures
                                   total-repeats meanq ease)

        (if (eql 'sm5 org-drill-fillin-spaced-repetition-algorithm)
            (setq org-drill-fillin-sm5-optimal-factor-matrix new-ofmatrix))
...
(defun org-drill-fillin-hypothetical-next-review-date (quality)
...
(defun org-drill-fillin-hypothetical-next-review-dates ()
  (let ((intervals nil))
    (dotimes (q 6)
      (push (max (or (car intervals) 0)
                 (org-drill-fillin-hypothetical-next-review-date q))
            intervals))
    (reverse intervals)))

(defun org-drill-fillin--read-key-sequence (prompt)
...
(defun org-drill-fillin-reschedule ()
  "Returns quality rating (0-5), or nil if the user quit."
  (let ((ch nil)
        (input nil)
        (next-review-dates (org-drill-fillin-hypothetical-next-review-dates))
        (typed-answer-statement (if drill-typed-answer
                                    (format "Your answer: %s\n"
                                            drill-typed-answer)
                                  ""))
        (key-prompt (format "(0-5, %c=help, %c=edit, %c=tags, %c=quit)"
                            org-drill-fillin--help-key
                            org-drill-fillin--edit-key
                            org-drill-fillin--tags-key
                            org-drill-fillin--quit-key)))
    (save-excursion
      (while (not (memq ch (list org-drill-fillin--quit-key
                                 org-drill-fillin--edit-key
                                 7          ; C-g
                                 ?0 ?1 ?2 ?3 ?4 ?5)))
        (setq input (org-drill-fillin--read-key-sequence
                     (if (eq ch org-drill-fillin--help-key)
...
%sHow well did you do? %s"
        ...
        (if (eql ch org-drill-fillin--tags-key)
            (org-set-tags-command))))
    (cond
     ((and (>= ch ?0) (<= ch ?5))
      (let ((quality (- ch ?0))
            (failures (org-drill-fillin-entry-failure-count)))
        (unless *org-drill-fillin-cram-mode*
          (save-excursion
            (let ((quality (if (org-drill-fillin--entry-lapsed-p) 2 quality)))
              (org-drill-fillin-smart-reschedule quality
                                          (nth quality next-review-dates))))
          (push quality *org-drill-fillin-session-qualities*)
          (cond
           ((<= quality org-drill-fillin-failure-quality)
            (when org-drill-fillin-leech-failure-threshold
              ;;(setq failures (if failures (string-to-number failures) 0))
              ;; (org-set-property "DRILL_FAILURE_COUNT"
              ;;                   (format "%d" (1+ failures)))
              (if (> (1+ failures) org-drill-fillin-leech-failure-threshold)
        ...
        quality))
     ((= ch org-drill-fillin--edit-key)
      'edit)
     (t
      nil))))


;; (defun org-drill-fillin-hide-all-subheadings-except (heading-list)
...
(defun org-drill-fillin-hide-subheadings-if (test)
...
(defun org-drill-fillin-hide-all-subheadings-except (heading-list)
  (org-drill-fillin-hide-subheadings-if
   (lambda () (let ((drill-heading (org-get-heading t)))
                (not (member drill-heading heading-list))))))


(defun org-drill-fillin--make-minibuffer-prompt (prompt)
  (let ((status (first (org-drill-fillin-entry-status)))
        (mature-entry-count (+ (length *org-drill-fillin-young-mature-entries*)
                               (length *org-drill-fillin-old-mature-entries*)
                               (length *org-drill-fillin-overdue-entries*))))
    (format "%s %s %s %s %s %s"
            (propertize
             (char-to-string
              (cond
               ((eql status :failed) ?F)
               (*org-drill-fillin-cram-mode* ?C)
               (t
                (case status
             ...
             'face `(:foreground
                     ,(case status
                        (:new org-drill-fillin-new-count-color)
                        ((:young :old) org-drill-fillin-mature-count-color)
                        ((:overdue :failed) org-drill-fillin-failed-count-color)
                        (t org-drill-fillin-done-count-color))))
            (propertize
             (number-to-string (length *org-drill-fillin-done-entries*))
             'face `(:foreground ,org-drill-fillin-done-count-color)
             'help-echo "The number of items you have reviewed this session.")
            (propertize
             (number-to-string (+ (length *org-drill-fillin-again-entries*)
                                  (length *org-drill-fillin-failed-entries*)))
             'face `(:foreground ,org-drill-fillin-failed-count-color)
             'help-echo (concat "The number of items that you failed, "
                                "and need to review again."))
            (propertize
             (number-to-string mature-entry-count)
             'face `(:foreground ,org-drill-fillin-mature-count-color)
             'help-echo "The number of old items due for review.")
            (propertize
             (number-to-string (length *org-drill-fillin-new-entries*))
             'face `(:foreground ,org-drill-fillin-new-count-color)
             'help-echo (concat "The number of new items that you "
                                "have never reviewed."))
            prompt)))


(cl-defun org-drill-fillin-presentation-prompt (&key prompt
                                              returns
         ...
         (prompt
          (or prompt
              (format (concat "Press key for answer, "
                      ...
                      org-drill-fillin--edit-key
                      org-drill-fillin--tags-key
                      org-drill-fillin--skip-key
                      org-drill-fillin--quit-key)))
         (full-prompt
          (org-drill-fillin--make-minibuffer-prompt prompt)))
    (if (and (eql 'warn org-drill-fillin-leech-method)
             (org-drill-fillin-entry-leech-p))
...
Consider reformulating the item to make it easier to remember.\n"
                                       'face '(:foreground "red"))
                           full-prompt)))
    (while (memq ch '(nil org-drill-fillin--tags-key))
      (setq ch nil)
      (while (not (input-pending-p))
        (let ((elapsed (time-subtract (current-time) item-start-time)))
          (message (concat (if (>= (time-to-seconds elapsed) (* 60 60))
                           ...
                           full-prompt))
          (sit-for 1)))
      (setq input (org-drill-fillin--read-key-sequence nil))
    ...
    (if returns
      ...
      (cond
       ((eql ch org-drill-fillin--quit-key) nil)
       ((eql ch org-drill-fillin--edit-key) 'edit)
       ((eql ch org-drill-fillin--skip-key) 'skip)
       (t t)))))


(cl-defun org-drill-fillin-presentation-prompt-for-string (prompt)
  "Create a card prompt with a timer and user-specified menu.
...
(defun org-drill-fillin-hide-region (beg end &optional text)
...
visual overlay, or with the string TEXT if it is supplied."
  (let ((ovl (make-overlay beg end)))
    (overlay-put ovl 'category
                 'org-drill-fillin-hidden-text-overlay)
...
(defun org-drill-fillin-hide-heading-at-point (&optional text)
  (unless (org-at-heading-p)
    (error "Point is not on a heading."))
  (save-excursion
    (let ((beg (point)))
      (end-of-line)
      (org-drill-fillin-hide-region beg (point) text))))


(defun org-drill-fillin-hide-comments ()
  (save-excursion
    (while (re-search-forward "^#.*$" nil t)
      (org-drill-fillin-hide-region (match-beginning 0) (match-end 0)))))


(defun org-drill-fillin-unhide-text ()
  ;; This will also unhide the item's heading.
  (save-excursion
    (dolist (ovl (overlays-in (point-min) (point-max)))
      (when (eql 'org-drill-fillin-hidden-text-overlay (overlay-get ovl 'category))
        (delete-overlay ovl)))))


(defun org-drill-fillin-hide-clozed-text ()
  (save-excursion
    (while (re-search-forward org-drill-fillin-cloze-regexp nil t)
      ;; Don't hide:
      ...
      (unless (save-match-data
        ...
        (org-drill-fillin-hide-matched-cloze-text)))))


(defun org-drill-fillin-hide-matched-cloze-text ()
  "Hide the current match with a 'cloze' visual overlay."
  (let ((ovl (make-overlay (match-beginning 0) (match-end 0)))
        (hint-sep-pos (string-match-p (regexp-quote org-drill-fillin-hint-separator)
                                      (match-string 0))))
    (overlay-put ovl 'category
                 'org-drill-fillin-cloze-overlay-defaults)
    (overlay-put ovl 'priority 9999)
    (if org-drill-fillin-cloze-length-matches-hidden-text-p
    ...
    (when (and hint-sep-pos
      ...
      (let ((hint (substring-no-properties
                   (match-string 0)
                   (+ hint-sep-pos (length org-drill-fillin-hint-separator))
...
(defun org-drill-fillin-hide-cloze-hints ()
  (save-excursion
    (while (re-search-forward org-drill-fillin-cloze-regexp nil t)
      (unless (or (save-match-data
        ...
        (org-drill-fillin-hide-region (match-beginning 2) (match-end 2))))))
...
concealed by an overlay that displays the string TEXT."
  `(progn
     (org-drill-fillin-replace-entry-text ,text)
     (unwind-protect
         (progn
           ,@body)
       (org-drill-fillin-unreplace-entry-text))))
...
concealed by an overlay that displays the overlays in REPLACEMENTS."
  `(progn
     (org-drill-fillin-replace-entry-text ,replacements t)
     (unwind-protect
         (progn
           ,@body)
       (org-drill-fillin-unreplace-entry-text))))
...
(defun org-drill-fillin-replace-entry-text (text &optional multi-p)
...
Note: does not actually alter the item."
  (cond
   ((and multi-p
         (listp text))
    (org-drill-fillin-replace-entry-text-multi text))
   (t
    (let ((ovl (make-overlay (point-min)
      ...
      (overlay-put ovl 'priority 9999)
      (overlay-put ovl 'category
                   'org-drill-fillin-replaced-text-overlay)
      (overlay-put ovl 'display text)))))


(defun org-drill-fillin-unreplace-entry-text ()
  (save-excursion
    (dolist (ovl (overlays-in (point-min) (point-max)))
      (when (eql 'org-drill-fillin-replaced-text-overlay (overlay-get ovl 'category))
        (delete-overlay ovl)))))


(defun org-drill-fillin-replace-entry-text-multi (replacements)
...
(defmacro with-replaced-entry-heading (heading &rest body)
  `(progn
     (org-drill-fillin-replace-entry-heading ,heading)
     (unwind-protect
         (progn
           ,@body)
       (org-drill-fillin-unhide-text))))
...
(defun org-drill-fillin-replace-entry-heading (heading)
  "Make an overlay that conceals the heading of the item. The overlay shows
the string TEXT.
Note: does not actually alter the item."
  (org-drill-fillin-hide-heading-at-point heading))


(defun org-drill-fillin-unhide-clozed-text ()
  (save-excursion
    (dolist (ovl (overlays-in (point-min) (point-max)))
      (when (eql 'org-drill-fillin-cloze-overlay-defaults (overlay-get ovl 'category))
        (delete-overlay ovl)))))


(defun org-drill-fillin-get-entry-text (&optional keep-properties-p)
...
;;   (zerop (length (org-drill-fillin-get-entry-text))))
...
(defun org-drill-fillin-entry-empty-p () (org-entry-empty-p))
...
(defun org-drill-fillin-present-simple-card ()
  (with-hidden-comments
   (with-hidden-cloze-hints
    (with-hidden-cloze-text
     (org-drill-fillin-hide-all-subheadings-except nil)
     (org-drill-fillin--show-latex-fragments)  ; overlay all LaTeX fragments with images
     (ignore-errors
       (org-display-inline-images t))
     (org-cycle-hide-drawers 'all)
     (prog1 (org-drill-fillin-presentation-prompt)
       (org-drill-fillin-hide-subheadings-if 'org-drill-fillin-entry-p))))))


(defun org-drill-fillin-present-default-answer (reschedule-fn)
  (prog1 (cond
          (drill-answer
          ...
          (t
           (org-drill-fillin-hide-subheadings-if 'org-drill-fillin-entry-p)
           (org-drill-fillin-unhide-clozed-text)
           (org-drill-fillin--show-latex-fragments)
           (ignore-errors
             (org-display-inline-images t))
           (org-cycle-hide-drawers 'all)
           (with-hidden-cloze-hints
...
(defun org-drill-fillin-present-simple-card-with-typed-answer ()
  (with-hidden-comments
   (with-hidden-cloze-hints
    (with-hidden-cloze-text
     (org-drill-fillin-hide-all-subheadings-except nil)
     ...
     (ignore-errors
       (org-display-inline-images t))
     (org-cycle-hide-drawers 'all)
     (prog1 (org-drill-fillin-presentation-prompt-for-string nil)
       (org-drill-fillin-hide-subheadings-if 'org-drill-fillin-entry-p))))))


(defun org-drill-fillin--show-latex-fragments ()
...
(defun org-drill-fillin-present-two-sided-card ()
  (with-hidden-comments
   (with-hidden-cloze-hints
    (with-hidden-cloze-text
     (let ((drill-sections (org-drill-fillin-hide-all-subheadings-except nil)))
       (when drill-sections
         (save-excursion
...
(defun org-drill-fillin-present-multi-sided-card ()
  (with-hidden-comments
   (with-hidden-cloze-hints
    (with-hidden-cloze-text
     (let ((drill-sections (org-drill-fillin-hide-all-subheadings-except nil)))
...
(defun org-drill-fillin-present-multicloze-hide-n (number-to-hide
                                            &optional
                                            force-show-first
                                            force-show-last
                                            force-hide-first)
   ...
   (with-hidden-cloze-hints
    (let ((item-end nil)
      ...
      (save-excursion
        ...
        (while (re-search-forward org-drill-fillin-cloze-regexp item-end t)
            ...
            (while (re-search-forward org-drill-fillin-cloze-regexp item-end t)
              (unless (save-match-data
                ...
                (if (memq cnt match-nums)
                    (org-drill-fillin-hide-matched-cloze-text)))))))
      ;; (loop
      ;;  do (re-search-forward org-drill-fillin-cloze-regexp
...
(defun org-drill-fillin-present-multicloze-hide-nth (to-hide)
...
(defun org-drill-fillin-present-multicloze-hide1 ()
  "Hides one of the pieces of text that are marked for cloze deletion,
chosen at random."
  (org-drill-fillin-present-multicloze-hide-n 1))


(defun org-drill-fillin-present-multicloze-hide2 ()
  "Hides two of the pieces of text that are marked for cloze deletion,
chosen at random."
  (org-drill-fillin-present-multicloze-hide-n 2))


(defun org-drill-fillin-present-multicloze-hide-first ()
  "Hides the first piece of text that is marked for cloze deletion."
  (org-drill-fillin-present-multicloze-hide-nth 1))


(defun org-drill-fillin-present-multicloze-hide-last ()
  "Hides the last piece of text that is marked for cloze deletion."
  (org-drill-fillin-present-multicloze-hide-nth -1))


(defun org-drill-fillin-present-multicloze-hide1-firstmore ()
...
the value of `org-drill-fillin-cloze-text-weight'."
  ...
  (cond
   ((null org-drill-fillin-cloze-text-weight)
    ;; Behave as hide1cloze
    (org-drill-fillin-present-multicloze-hide1))
   ((not (and (integerp org-drill-fillin-cloze-text-weight)
              (plusp org-drill-fillin-cloze-text-weight)))
    (error "Illegal value for org-drill-fillin-cloze-text-weight: %S"
           org-drill-fillin-cloze-text-weight))
   ((zerop (mod (1+ (org-drill-fillin-entry-total-repeats 0))
                org-drill-fillin-cloze-text-weight))
    ;; Uncommonly, hide any item except the first
    (org-drill-fillin-present-multicloze-hide-n 1 t))
   (t
    ;; Commonly, hide first item
    (org-drill-fillin-present-multicloze-hide-first))))


(defun org-drill-fillin-present-multicloze-show1-lastmore ()
...
the value of `org-drill-fillin-cloze-text-weight'."
  (cond
   ((null org-drill-fillin-cloze-text-weight)
    ...
    (org-drill-fillin-present-multicloze-show1))
   ((not (and (integerp org-drill-fillin-cloze-text-weight)
              (plusp org-drill-fillin-cloze-text-weight)))
    (error "Illegal value for org-drill-fillin-cloze-text-weight: %S"
           org-drill-fillin-cloze-text-weight))
   ((zerop (mod (1+ (org-drill-fillin-entry-total-repeats 0))
    ...
    (org-drill-fillin-present-multicloze-hide-n -1 nil nil t))
   (t
    ;; Commonly, show the LAST item
    (org-drill-fillin-present-multicloze-hide-n -1 nil t))))


(defun org-drill-fillin-present-multicloze-show1-firstless ()
...
(defun org-drill-fillin-present-multicloze-show1 ()
  "Similar to `org-drill-fillin-present-multicloze-hide1', but hides all
the pieces of text that are marked for cloze deletion, except for one
piece which is chosen at random."
  (org-drill-fillin-present-multicloze-hide-n -1))


(defun org-drill-fillin-present-multicloze-show2 ()
  "Similar to `org-drill-fillin-present-multicloze-show1', but reveals two
pieces rather than one."
  (org-drill-fillin-present-multicloze-hide-n -2))


(defun org-drill-fillin-present-card-using-text (question &optional answer)
...
(defun org-drill-fillin-present-card-using-multiple-overlays (replacements &optional answer)
...
(defun org-drill-fillin-entry ()
  "Present the current topic for interactive review, as in `org-drill-fillin'.
Review will occur regardless of whether the topic is due for review or whether
it meets the definition of a 'review topic' used by `org-drill-fillin'.
...
later using `org-drill-fillin-resume'.

See `org-drill-fillin' for more details."
  (interactive)
  (org-drill-fillin-goto-drill-entry-heading)
  ;;(unless (org-part-of-drill-entry-p)
  ;;  (error "Point is not inside a drill entry"))
  ;;(unless (org-at-heading-p)
  ;;  (org-back-to-heading))
  (let ((card-type (org-entry-get (point) "DRILL_CARD_TYPE" t))
        (answer-fn 'org-drill-fillin-present-default-answer)
    ...
    (org-save-outline-visibility t
      (save-restriction
        ...
        (let ((presentation-fn
               (cdr (assoc card-type org-drill-fillin-card-type-alist))))
          (if (listp presentation-fn)
              (psetq answer-fn (or (second presentation-fn)
                                   'org-drill-fillin-present-default-answer)
               ...
               (t
                ...
                (cond
                 ...
                 (t
                  (save-excursion
                    (funcall answer-fn
                             (lambda () (org-drill-fillin-reschedule))))))))
            (org-remove-latex-fragment-image-overlays)))))))


(defun org-drill-fillin-entries-pending-p ()
  (or *org-drill-fillin-again-entries*
      *org-drill-fillin-current-item*
      (and (not (org-drill-fillin-maximum-item-count-reached-p))
           (not (org-drill-fillin-maximum-duration-reached-p))
           (or *org-drill-fillin-new-entries*
               *org-drill-fillin-failed-entries*
               *org-drill-fillin-young-mature-entries*
               *org-drill-fillin-old-mature-entries*
               *org-drill-fillin-overdue-entries*
               *org-drill-fillin-again-entries*))))


(defun org-drill-fillin-pending-entry-count ()
  (+ (if (markerp *org-drill-fillin-current-item*) 1 0)
     (length *org-drill-fillin-new-entries*)
     (length *org-drill-fillin-failed-entries*)
     (length *org-drill-fillin-young-mature-entries*)
     (length *org-drill-fillin-old-mature-entries*)
     (length *org-drill-fillin-overdue-entries*)
     (length *org-drill-fillin-again-entries*)))


(defun org-drill-fillin-maximum-duration-reached-p ()
  "Returns true if the current drill session has continued past its
maximum duration."
  (and org-drill-fillin-maximum-duration
       (not *org-drill-fillin-cram-mode*)
       *org-drill-fillin-start-time*
       (> (- (float-time (current-time)) *org-drill-fillin-start-time*)
          (* org-drill-fillin-maximum-duration 60))))


(defun org-drill-fillin-maximum-item-count-reached-p ()
  "Returns true if the current drill session has reached the
maximum number of items."
  (and org-drill-fillin-maximum-items-per-session
       (not *org-drill-fillin-cram-mode*)
       (>= (if org-drill-fillin-item-count-includes-failed-items-p
               (+ (length *org-drill-fillin-done-entries*)
                  (length *org-drill-fillin-again-entries*))
             (length *org-drill-fillin-done-entries*))
           org-drill-fillin-maximum-items-per-session)))


(defun org-drill-fillin-pop-next-pending-entry ()
  (block org-drill-fillin-pop-next-pending-entry
    (let ((m nil))
      (while (or (null m)
                 (not (org-drill-fillin-entry-p m)))
        (setq
         m
         (cond
          ;; First priority is items we failed in a prior session.
          ((and *org-drill-fillin-failed-entries*
                (not (org-drill-fillin-maximum-item-count-reached-p))
                (not (org-drill-fillin-maximum-duration-reached-p)))
           (pop-random *org-drill-fillin-failed-entries*))
          ;; Next priority is overdue items.
          ((and *org-drill-fillin-overdue-entries*
                (not (org-drill-fillin-maximum-item-count-reached-p))
                (not (org-drill-fillin-maximum-duration-reached-p)))
           ...
           (pop *org-drill-fillin-overdue-entries*))
          ;; Next priority is 'young' items.
          ((and *org-drill-fillin-young-mature-entries*
                (not (org-drill-fillin-maximum-item-count-reached-p))
                (not (org-drill-fillin-maximum-duration-reached-p)))
           (pop-random *org-drill-fillin-young-mature-entries*))
          ;; Next priority is newly added items, and older entries.
          ;; We pool these into a single group.
          ((and (or *org-drill-fillin-new-entries*
                    *org-drill-fillin-old-mature-entries*)
                (not (org-drill-fillin-maximum-item-count-reached-p))
                (not (org-drill-fillin-maximum-duration-reached-p)))
           (cond
            ((< (random* (+ (length *org-drill-fillin-new-entries*)
                            (length *org-drill-fillin-old-mature-entries*)))
                (length *org-drill-fillin-new-entries*))
             (pop-random *org-drill-fillin-new-entries*))
            (t
             (pop-random *org-drill-fillin-old-mature-entries*))))
          ;; After all the above are done, last priority is items
          ;; that were failed earlier THIS SESSION.
          (*org-drill-fillin-again-entries*
           (pop *org-drill-fillin-again-entries*))
          (t                            ; nothing left -- return nil
           (return-from org-drill-fillin-pop-next-pending-entry nil)))))
      m)))


(defun org-drill-fillin-entries (&optional resuming-p)
...
RESUMING-P is true if we are resuming a suspended drill session."
  (block org-drill-fillin-entries
    (while (org-drill-fillin-entries-pending-p)
      (let ((m (cond
                ((or (not resuming-p)
                     (null *org-drill-fillin-current-item*)
                     (not (org-drill-fillin-entry-p *org-drill-fillin-current-item*)))
                 (org-drill-fillin-pop-next-pending-entry))
                (t                      ; resuming a suspended session.
                 (setq resuming-p nil)
                 *org-drill-fillin-current-item*))))
        (setq *org-drill-fillin-current-item* m)
        (unless m
          (error "Unexpectedly ran out of pending drill items"))
        (save-excursion
          (org-drill-fillin-goto-entry m)
          (cond
           ((not (org-drill-fillin-entry-due-p))
            ...
            nil)
           (t
            (setq result (org-drill-fillin-entry))
            (cond
             ((null result)
              (message "Quit")
              (setq end-pos :quit)
              (return-from org-drill-fillin-entries nil))
             ((eql result 'edit)
              (setq end-pos (point-marker))
              (return-from org-drill-fillin-entries nil))
             ((eql result 'skip)
              (setq *org-drill-fillin-current-item* nil)
              nil)                      ; skip this item
             (t
              (cond
               ((<= result org-drill-fillin-failure-quality)
                (if *org-drill-fillin-again-entries*
                    (setq *org-drill-fillin-again-entries*
                          (shuffle-list *org-drill-fillin-again-entries*)))
                (push-end m *org-drill-fillin-again-entries*))
               (t
                (push m *org-drill-fillin-done-entries*)))
              (setq *org-drill-fillin-current-item* nil))))))))))



(defun org-drill-fillin-final-report ()
  (let ((pass-percent
         (round (* 100 (count-if (lambda (qual)
                                   (> qual org-drill-fillin-failure-quality))
                                 *org-drill-fillin-session-qualities*))
                (max 1 (length *org-drill-fillin-session-qualities*))))
        (prompt nil)
...
Session finished. Press a key to continue..."
           ...
           (format-seconds "%h:%.2m:%.2s"
                           (- (float-time (current-time)) *org-drill-fillin-start-time*))
           (round (* 100 (count 5 *org-drill-fillin-session-qualities*))
                  (max 1 (length *org-drill-fillin-session-qualities*)))
           (round (* 100 (count 2 *org-drill-fillin-session-qualities*))
                  (max 1 (length *org-drill-fillin-session-qualities*)))
           (round (* 100 (count 4 *org-drill-fillin-session-qualities*))
                  (max 1 (length *org-drill-fillin-session-qualities*)))
           (round (* 100 (count 1 *org-drill-fillin-session-qualities*))
                  (max 1 (length *org-drill-fillin-session-qualities*)))
           (round (* 100 (count 3 *org-drill-fillin-session-qualities*))
                  (max 1 (length *org-drill-fillin-session-qualities*)))
           (round (* 100 (count 0 *org-drill-fillin-session-qualities*))
                  (max 1 (length *org-drill-fillin-session-qualities*)))
           pass-percent
           org-drill-fillin-failure-quality
           (org-drill-fillin-pending-entry-count)
           (+ (org-drill-fillin-pending-entry-count)
              *org-drill-fillin-dormant-entry-count*)
           (propertize
            (format "%d failed"
                    (+ (length *org-drill-fillin-failed-entries*)
                       (length *org-drill-fillin-again-entries*)))
           ...
           (propertize
            (format "%d old"
                    (length *org-drill-fillin-old-mature-entries*))
            'face `(:foreground ,org-drill-fillin-mature-count-color))
           *org-drill-fillin-due-tomorrow-count*
           ))
    ...
    (if (and *org-drill-fillin-session-qualities*
             (< pass-percent (- 100 org-drill-fillin-forgetting-index)))
        (read-char-exclusive
         (format
          "%s
...
lowering the value of `org-drill-fillin-learn-fraction' slightly in
order to make items appear more frequently over time."
          (propertize "WARNING!" 'face 'org-warning)
          (- 100 pass-percent)
          *org-drill-fillin-overdue-entry-count*
          (round (* 100 *org-drill-fillin-overdue-entry-count*)
                 (+ *org-drill-fillin-dormant-entry-count*
                    *org-drill-fillin-due-entry-count*)))
         ))))



(defun org-drill-fillin-free-markers (markers)
  "MARKERS is a list of markers, all of which will be freed (set to
point nowhere). Alternatively, MARKERS can be 't', in which case
all the markers used by Org-Drill-Fillin will be freed."
  (dolist (m (if (eql t markers)
                 (append  *org-drill-fillin-done-entries*
                          *org-drill-fillin-new-entries*
                          *org-drill-fillin-failed-entries*
                          *org-drill-fillin-again-entries*
                          *org-drill-fillin-overdue-entries*
                          *org-drill-fillin-young-mature-entries*
...
(defun org-drill-fillin-order-overdue-entries (overdue-data)
  (let* ((lapsed-days (if org-drill-fillin--lapse-very-overdue-entries-p
    ...
    (setq *org-drill-fillin-overdue-entries*
          (mapcar 'first
                  (append
...
(defun org-drill-fillin--entry-lapsed-p ()
  (let ((lapsed-days 90))
    (and org-drill-fillin--lapse-very-overdue-entries-p
         (> (or (org-drill-fillin-entry-days-overdue) 0) lapsed-days))))




(defun org-drill-fillin-entry-days-since-creation (&optional use-last-interval-p)
...
that many days)."
  (let ((timestamp (org-entry-get (point) "DATE_ADDED")))
    (cond
     ...
     (use-last-interval-p
      (+ (or (org-drill-fillin-entry-days-overdue) 0)
         (read (or (org-entry-get (point) "DRILL_LAST_INTERVAL") "0"))))
     (t nil))))


(defun org-drill-fillin-entry-status ()
...
"
  (save-excursion
    (unless (org-at-heading-p)
      (org-back-to-heading))
    (let ((due (org-drill-fillin-entry-days-overdue))
          (age (org-drill-fillin-entry-days-since-creation t))
          (last-int (org-drill-fillin-entry-last-interval 1)))
      (list
       (cond
        ((not (org-drill-fillin-entry-p))
         nil)
        ((and (org-entry-empty-p)
              (let* ((card-type (org-entry-get (point) "DRILL_CARD_TYPE" nil))
                     (dat (cdr (assoc card-type org-drill-fillin-card-type-alist))))
                (or (null card-type)
                    (not (third dat)))))
...
(defun org-drill-fillin-progress-message (collected scanned)
...
(defun org-map-drill-entry-function ()
  (org-drill-fillin-progress-message
   (+ (length *org-drill-fillin-new-entries*)
      (length *org-drill-fillin-overdue-entries*)
      (length *org-drill-fillin-young-mature-entries*)
      (length *org-drill-fillin-old-mature-entries*)
  ...
  (cond
   ...
   (t
    ...
    (destructuring-bind (status due age)
        (org-drill-fillin-entry-status)
      (case status
        (:unscheduled
         (incf *org-drill-fillin-dormant-entry-count*))
        ;; (:tomorrow
        ;;  (incf *org-drill-fillin-dormant-entry-count*)
        ;;  (incf *org-drill-fillin-due-tomorrow-count*))
        (:future
         (incf *org-drill-fillin-dormant-entry-count*)
         (if (eq -1 due)
             (incf *org-drill-fillin-due-tomorrow-count*)))
        (:new
         (push (point-marker) *org-drill-fillin-new-entries*))
        (:failed
         (push (point-marker) *org-drill-fillin-failed-entries*))
        (:young
         (push (point-marker) *org-drill-fillin-young-mature-entries*))
        (:overdue
         (push (list (point-marker) due age) overdue-data))
        (:old
         (push (point-marker) *org-drill-fillin-old-mature-entries*))
        )))))


(defun org-drill-fillin (&optional scope drill-match resume-p)
...
- If `org-drill-fillin-maximum-items-per-session' is set, a random
...
questions.  It accepts the same values as `org-drill-fillin-scope',
which see.
...
It accepts the same values as `org-drill-fillin-match', which see.
...
than starting a new one."
  ...
  (let ((majorv (first (mapcar 'string-to-number (split-string (org-release) "[.]")))))
    (if (and (< majorv 8)
        ...
        (read-char-exclusive
         (format "Warning: org-drill-fillin requires org mode 7.9.3f or newer. Scheduling of failed cards will not
...
7.9.3f. Please consider installing a more recent version of org mode." (org-release)))))
  (let ((end-pos nil)
        (overdue-data nil)
        (cnt 0))
    (block org-drill-fillin
      (unless resume-p
        (org-drill-fillin-free-markers t)
        (setq *org-drill-fillin-current-item* nil
              *org-drill-fillin-done-entries* nil
              *org-drill-fillin-dormant-entry-count* 0
              *org-drill-fillin-due-entry-count* 0
              *org-drill-fillin-due-tomorrow-count* 0
              *org-drill-fillin-overdue-entry-count* 0
              *org-drill-fillin-new-entries* nil
              *org-drill-fillin-overdue-entries* nil
              *org-drill-fillin-young-mature-entries* nil
              *org-drill-fillin-old-mature-entries* nil
              *org-drill-fillin-failed-entries* nil
              *org-drill-fillin-again-entries* nil)
        (setq *org-drill-fillin-session-qualities* nil)
        (setq *org-drill-fillin-start-time* (float-time (current-time))))
            ...
            (unless resume-p
              (let ((org-trust-scanner-tags t)
                ...
                (org-map-drill-entries
                 ...
                 scope drill-match)
                (org-drill-fillin-order-overdue-entries overdue-data)
                (setq *org-drill-fillin-overdue-entry-count*
                      (length *org-drill-fillin-overdue-entries*))))
            (setq *org-drill-fillin-due-entry-count* (org-drill-fillin-pending-entry-count))
            (cond
             ((and (null *org-drill-fillin-current-item*)
                   (null *org-drill-fillin-new-entries*)
                   (null *org-drill-fillin-failed-entries*)
                   (null *org-drill-fillin-overdue-entries*)
                   (null *org-drill-fillin-young-mature-entries*)
                   (null *org-drill-fillin-old-mature-entries*))
              (message "I did not find any pending drill items."))
             (t
              (org-drill-fillin-entries resume-p)
              (message "Drill session finished!"))))
        (progn
          (unless end-pos
            (setq *org-drill-fillin-cram-mode* nil)
            (org-drill-fillin-free-markers *org-drill-fillin-done-entries*)))))
    (cond
     (end-pos
      (when (markerp end-pos)
        (org-drill-fillin-goto-entry end-pos)
        (org-reveal)
        (org-show-entry))
      (let ((keystr (command-keybinding-to-string 'org-drill-fillin-resume)))
        (message
         "You can continue the drill session with the command `org-drill-fillin-resume'.%s"
         (if keystr (format "\nYou can run this command by pressing %s." keystr)
           ""))))
     (t
      (org-drill-fillin-final-report)
      (if (eql 'sm5 org-drill-fillin-spaced-repetition-algorithm)
          (org-drill-fillin-save-optimal-factor-matrix))
      (if org-drill-fillin-save-buffers-after-drill-sessions-p
          (save-some-buffers))
      (message "Drill session finished!")
      ))))


(defun org-drill-fillin-save-optimal-factor-matrix ()
  (savehist-autosave))


(defun org-drill-fillin-cram (&optional scope drill-match)
...
have been reviewed within the last `org-drill-fillin-cram-hours'
hours."
  (interactive)
  (setq *org-drill-fillin-cram-mode* t)
  (org-drill-fillin scope drill-match))


(defun org-drill-fillin-tree ()
  "Run an interactive drill session using drill items within the
subtree at point."
  (interactive)
  (org-drill-fillin 'tree))


(defun org-drill-fillin-directory ()
  "Run an interactive drill session using drill items from all org
files in the same directory as the current file."
  (interactive)
  (org-drill-fillin 'directory))


(defun org-drill-fillin-again (&optional scope drill-match)
...
scan will be performed."
  ...
  (setq *org-drill-fillin-cram-mode* nil)
  (cond
   ((plusp (org-drill-fillin-pending-entry-count))
    (org-drill-fillin-free-markers *org-drill-fillin-done-entries*)
    (if (markerp *org-drill-fillin-current-item*)
        (free-marker *org-drill-fillin-current-item*))
    (setq *org-drill-fillin-start-time* (float-time (current-time))
          *org-drill-fillin-done-entries* nil
          *org-drill-fillin-current-item* nil)
    (org-drill-fillin scope drill-match t))
   (t
    (org-drill-fillin scope drill-match))))



(defun org-drill-fillin-resume ()
  "Resume a suspended drill session. Sessions are suspended by
exiting them with the `edit' or `quit' options."
  (interactive)
  (cond
   ((org-drill-fillin-entries-pending-p)
    (org-drill-fillin nil nil t))
   ((and (plusp (org-drill-fillin-pending-entry-count))
...
need reviewing. Start a new drill session? "
                    (org-drill-fillin-pending-entry-count))))
    (org-drill-fillin-again))
   (t
    (message "You have finished the drill session."))))


(defun org-drill-fillin-relearn-item ()
...
failure. This command can be used to 'reset' repetitions for an item."
  (interactive)
  (org-drill-fillin-smart-reschedule 4 0))


(defun org-drill-fillin-strip-entry-data ()
  (dolist (prop org-drill-fillin-scheduling-properties)
    (org-delete-property prop))
  (org-schedule '(4)))


(defun org-drill-fillin-strip-all-data (&optional scope)
...
values as `org-drill-fillin-scope'."
  (interactive)
  (when (yes-or-no-p
    ...
    (cond
     ...
     (t
      (org-map-drill-entries 'org-drill-fillin-strip-entry-data scope)))
    (message "Done.")))


(defun org-drill-fillin-add-cloze-fontification ()
  ;; Compute local versions of the regexp for cloze deletions, in case
  ;; the left and right delimiters are redefined locally.
  (setq-local org-drill-fillin-cloze-regexp (org-drill-fillin--compute-cloze-regexp))
  (setq-local org-drill-fillin-cloze-keywords (org-drill-fillin--compute-cloze-keywords))
  (when org-drill-fillin-use-visible-cloze-face-p
    (add-to-list 'org-font-lock-extra-keywords
                 (first org-drill-fillin-cloze-keywords))))
...
;;                               org-drill-fillin-cloze-keywords
;;                               nil))))

;; XXX
;; (add-hook 'hack-local-variables-hook
;;           'org-drill-fillin-add-cloze-fontification)
;;
;; (org-drill-fillin-add-cloze-fontification)
...
(defvar *org-drill-fillin-dest-id-table* (make-hash-table :test 'equal))


(defun org-drill-fillin-copy-entry-to-other-buffer (dest &optional path)
  "Copy the subtree at point to the buffer DEST. The copy will receive
the tag 'imported'."
  (block org-drill-fillin-copy-entry-to-other-buffer
    (save-excursion
      (let ((src (current-buffer))
            (m nil))
        (cl-flet ((paste-tree-here (&optional level)
                                   (org-paste-subtree level)
                                   (org-drill-fillin-strip-entry-data)
                                   (org-toggle-tag "imported" 'on)
                                   (org-map-drill-entries
                                    (lambda ()
                                      (let ((id (org-id-get)))
                                        (org-drill-fillin-strip-entry-data)
                                        (unless (gethash id *org-drill-fillin-dest-id-table*)
                                          (puthash id (point-marker)
                                                   *org-drill-fillin-dest-id-table*))))
                                    'tree)))
          (unless path
          ...
          (setq m
                (condition-case nil
                  ...
                  (error                ; path does not exist in DEST
                   (return-from org-drill-fillin-copy-entry-to-other-buffer
                     (cond
                      ((cdr path)
                       (org-drill-fillin-copy-entry-to-other-buffer
                        dest (butlast path)))
                      (t
...
(defun org-drill-fillin-merge-buffers (src &optional dest ignore-new-items-p)
...
copy them across."
  ...
  (when (yes-or-no-p
    ...
    (clrhash *org-drill-fillin-dest-id-table*)
    (with-current-buffer dest
      (org-map-drill-entries
       (lambda ()
         (let ((this-id (org-id-get)))
           (when this-id
             (puthash this-id (point-marker) *org-drill-fillin-dest-id-table*))))
       'file))
           ...
           (cond
            ...
            ((gethash id *org-drill-fillin-dest-id-table*)
             ...
             (let ((marker (gethash id *org-drill-fillin-dest-id-table*)))
               (destructuring-bind (last-interval repetitions failures
                 ...
                 (save-excursion
                   ...
                   (unless (zerop total-repeats)
                     (org-drill-fillin-store-item-data last-interval repetitions failures
                                                       total-repeats meanq ease)
                     (if last-quality
               ...
               (remhash id *org-drill-fillin-dest-id-table*)
               (free-marker marker)))
            (t
             ...
             (unless ignore-new-items-p
               (org-drill-fillin-copy-entry-to-other-buffer dest))))))
       'file))
...
(defvar org-drill-fillin-verb-tense-alist
  '(("present" "tomato")
...
which will be used by `org-drill-fillin-present-verb-conjugation' and
`org-drill-fillin-show-answer-verb-conjugation' to fontify the verb and
the name of the tense.")


(defun org-drill-fillin-get-verb-conjugation-info ()
  "Auxiliary function used by `org-drill-fillin-present-verb-conjugation' and
`org-drill-fillin-show-answer-verb-conjugation'."
  (let ((infinitive (org-entry-get (point) "VERB_INFINITIVE" t))
    ...
    (setq highlight-face
          (list :foreground
                (or (second (assoc-string tense org-drill-fillin-verb-tense-alist t))
                    "hotpink")
                :background
                (second (assoc-string mood org-drill-fillin-verb-tense-alist t))))
...
(defun org-drill-fillin-present-verb-conjugation ()
  ...
  (cl-flet ((tense-and-mood-to-string
    ...
    (destructuring-bind (infinitive inf-hint translation tense mood)
        (org-drill-fillin-get-verb-conjugation-info)
      (org-drill-fillin-present-card-using-text
       (cond
        ((zerop (random* 2))
...
(defun org-drill-fillin-show-answer-verb-conjugation (reschedule-fn)
  "Show the answer for a drill item whose card type is 'conjugate'.
RESCHEDULE-FN must be a function that calls `org-drill-fillin-reschedule' and
...
(defvar org-drill-fillin-noun-gender-alist
...
(defun org-drill-fillin-get-noun-info ()
  "Auxiliary function used by `org-drill-fillin-present-noun-declension' and
`org-drill-fillin-show-answer-noun-declension'."
    ...
    (setq highlight-face
          (list :foreground
                (or (second (assoc-string noun-gender
                                          org-drill-fillin-noun-gender-alist t))
                    "red")))
...
(defun org-drill-fillin-show-answer-noun-declension (reschedule-fn)
...
(defun org-drill-fillin-present-translate-number ()
...
;; (defun org-drill-fillin-show-answer-translate-number (reschedule-fn)
...
(defun org-drill-fillin-present-spanish-verb ()
  (let ((prompt nil)
        (reveal-headings nil))
    (with-hidden-comments
     (with-hidden-cloze-hints
      (with-hidden-cloze-text
       (case (random* 6)
         (0
          (org-drill-fillin-hide-all-subheadings-except '("Infinitive"))
          (setq prompt
                (concat "Translate this Spanish verb, and conjugate it "
                        "for the *present* tense.")
                reveal-headings '("English" "Present Tense" "Notes")))
         (1
          (org-drill-fillin-hide-all-subheadings-except '("English"))
